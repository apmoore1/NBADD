...
class BivalencyAttentionDialectClassifierTest(ModelTestCase):
    def setUp(self):
        super().setUp()
        test_data_dir = Path(__file__, '..', '..', 'test_data')
        test_data_fp = str(Path(test_data_dir, 'aoc_test_data.csv').resolve())
        test_model_fp = str(Path(test_data_dir, 'bivalency_attention_dialect_classifier.json').resolve())
        self.set_up_model(test_model_fp, test_data_fp)
...
class CodeSwitchingAndBivalencyAttentionDialectClassifierTest(ModelTestCase):
    def setUp(self):
        super().setUp()
        test_data_dir = Path(__file__, '..', '..', 'test_data')
        test_data_fp = str(Path(test_data_dir, 'aoc_test_data.csv').resolve())
        test_model_fp = str(Path(test_data_dir, 'code_switching_bivalency_attention_dialect_classifier.json').resolve())
        self.set_up_model(test_model_fp, test_data_fp)
    ...
    def test_code_switching_loss(self):
        ...
        # Training loss should not equal validation loss as the training loss 
        # should include the additional attention regulisation penalty the 
        # the different will be arount 1.3
        with pytest.raises(AssertionError):
            numpy.testing.assert_almost_equal(training_loss, validation_loss, decimal=1)
        assert training_loss- validation_loss > 1